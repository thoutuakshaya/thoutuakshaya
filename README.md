

<!-- GitHub Profile README: Thoutu Akshaya -->

<h1 align="center">Hi 👋, I'm Thoutu Akshaya</h1>
<h3 align="center">💻 Passionate Engineering Student | Full-Stack Developer | Tech Explorer</h3>

---

🌟 **About Me**

Field: Engineering & Technology
Interests: Coding 💻 | AI 🤖 | Cloud ☁️ | Creative Tech 🎨
Goal: Build innovative tech solutions & grow continuously 🚀

| Project                           | Description                                   |
| --------------------------------- | --------------------------------------------- |
| **🌐 Portfolio Website**          | Built with Three.js & full-stack technologies |
| **🛠️ MERN CRUD App**             | MongoDB + Express + React + Node.js           |
| **📋 Employee Management System** | Spring Boot + MySQL project                   |
| **💬 AI Chatbot**                 | Created using GUVI platform tools             |
| **🎮 Tic Tac Toe**                | Python-based simple GUI game                  |


<p align="center"> <img src="https://github-readme-streak-stats.herokuapp.com/?user=ThoutuAkshaya&theme=radical&hide_border=true&stroke=0000&ring=FF6B81" alt="GitHub Streak Stats" /> </p> <p align="center"> <img src="https://github-readme-stats.vercel.app/api?username=ThoutuAkshaya&show_icons=true&theme=tokyonight&count_private=true&hide_border=true" /> </p> <p align="center"> <img src="https://github-readme-stats.vercel.app/api/top-langs/?username=ThoutuAkshaya&layout=compact&theme=tokyonight&hide_border=true" /> </p>



